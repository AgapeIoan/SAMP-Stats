...
from functii import *
from samp_server_stats import get_server_data, format_server_data
...
class RubyBot(commands.Cog):
    ...
    async def aplicanti(self, ctx, *, factiune_sau_id):
        ...
        async with ctx.channel.typing():
            ...
            with requests.Session() as s:
                ...
                aplicatii_noi = int(aplicatii[0].text[aplicatii[0].text.find("(")+1:-2]) # Putin cam strans dar in esenta
    ...
    async def clan(self, ctx, *, clan_sau_tag: str):
        ...
        async with ctx.channel.typing():
            ...
            with requests.Session() as s:
                ...
                descriere_raw, f2, soup = scrape_panou(s, url, [{'div': {'id': 'clanDescriptionText'}}, {'div': {'class': 'col-xs-3'}}], True)
                ...
                for i in range(3):  # Sarim peste primele 3 elemente
                    next(iteme)

                for valori in iteme:  # Iteram prind dictionar
                    contor += 1
    ...
    async def raportu(self, ctx, *, server=None):
        ...
        async with ctx.channel.typing():
            ...
            embed = discord.Embed(title="SAMP Servers", color=0x00ff00)
            with open("servers_dns.json", "r") as f:
                servers = json.load(f)
            ...
            if server:
                ...
                if server.find(".") == -1: # Nu avem ".", adica nu este corect specificat server-ul
                    await ctx.send(f"Serverul **{server}** nu a fost gasit. Verifica daca ai introdus corect adresa sau numele serverului!")
                    return
